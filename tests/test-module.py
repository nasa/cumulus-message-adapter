...
class Test(unittest.TestCase):  # pylint: disable=too-many-public-methods
    ...
    event_with_cma = {
        "cma": {"foo": "bar", "event": {"some": "object"}}
    }
    event_with_replace = {
        "replace": {"Bucket": bucket_name, "Key": key_name, "TargetPath": "$"}
    }
    ...
    def setUp(self) -> None:
        ...
        self.s3.Object(self.bucket_name, self.key_name).put(
            Body=json.dumps(self.s3_object)
        )
    ...
    def test_load_and_update_remote_event_does_not_overwrite_configuration(
        self,
    ) -> None:
        ...
        expected = {
            "task_config": self.config_event_with_replace["cma"]["task_config"],
            "input": ":blue_whale:",
        ...
        nested_event_local = {
            "task_config": {
                ...
                "cumulus_message": {
                    "input": "{$.payload.input}",
                    "outputs": [
                        {"source": "{$.input.anykey}", "destination": "{$.payload.out}"}
                    ],
                },
            },
    ...
    def test_result_payload_with_simple_config_outputs(self) -> None:
        ...
        message_config_with_simple_outputs = {
            "outputs": [
                {"source": "{$.input.dataLocation}", "destination": "{$.payload}"}
            ]
        }
    ...
    def test_result_payload_with_nested_sibling_config_outputs(self) -> None:
        ...
        assert result["test"] == {"dataLocation": "s3://source.jpg", "key": "value"}
    ...
    @patch("uuid.uuid4")
    def test_configured_big_result_stored_remotely(self, uuid_mock) -> None:
        """
        ...
        """
        event_with_ingest = {
            "cumulus_meta": {"workflow": "testing", "system_bucket": self.bucket_name},
        ...
        expected_remote_event_object = {"input": {"dataLocation": "s3://source.jpg"}}
        ...
        remote_event = self.s3.Object(
            self.bucket_name, self.next_event_object_key_name
        ).get()
        remote_event_object = json.loads(remote_event["Body"].read().decode("utf-8"))
    ...
    @patch("uuid.uuid4")
    def test_configured_big_result_with_non_dict_target_stored_remotely(
        self, uuid_mock
    ) -> None:
        """
        ...
        event_with_ingest = {
            "cumulus_meta": {"workflow": "testing", "system_bucket": self.bucket_name},
            "meta": {"another_key": {"some_meta_key": "some_meta_object"}},
            "ReplaceConfig": {
        ...
        expected_create_next_event_result = {
            "cumulus_meta": {"workflow": "testing", "system_bucket": self.bucket_name},
            "meta": {"another_key": {"some_meta_key": ""}},
    ...
    @patch("uuid.uuid4")
    def test_big_result_stored_remotely(self, uuid_mock) -> None:
        """
    ...
    def test_basic(self) -> None:
        ...
        inputFile = os.path.join(self.test_folder, "basic.input.json")
        outputFile = os.path.join(self.test_folder, "basic.output.json")
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
        ...
        result = self.cumulus_message_adapter.create_next_event(
            msg, in_msg, message_config
        )
        assert result == out_msg

    def test_exception(self) -> None:
        """test exception.input.json"""
        inputFile = os.path.join(self.test_folder, "exception.input.json")
        outputFile = os.path.join(self.test_folder, "exception.output.json")
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
            out_msg = json.load(out)
        ...
        with open(data_filename, "r", encoding="utf-8") as file_data:
            ...
            self.s3.Object(bucket_name, key_name).put(Body=file_data.read())

        remote_event = self.cumulus_message_adapter.load_and_update_remote_event(
            in_msg, {}
        )
        ...
        if "messageConfig" in msg:
            del msg["messageConfig"]
        result = self.cumulus_message_adapter.create_next_event(
            msg, remote_event, message_config
        )
        self.s3.Bucket(bucket_name).delete_objects(
            Delete={"Objects": [{"Key": key_name}]}
        )
    ...
    def test_jsonpath(self) -> None:
        ...
        inputFile = os.path.join(self.test_folder, "jsonpath.input.json")
        outputFile = os.path.join(self.test_folder, "jsonpath.output.json")
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
            out_msg = json.load(out)
    ...
    def test_meta(self) -> None:
        ...
        inputFile = os.path.join(self.test_folder, "meta.input.json")
        outputFile = os.path.join(self.test_folder, "meta.output.json")
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
            out_msg = json.load(out)
    ...
    def test_remote(self) -> None:
        ...
        inputFile = os.path.join(self.test_folder, "remote.input.json")
        outputFile = os.path.join(self.test_folder, "remote.output.json")
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
            out_msg = json.load(out)
    ...
    def test_configured_remote(self) -> None:
        ...
        inputFile = os.path.join(self.test_folder, "configured_remote.input.json")
        outputFile = os.path.join(self.test_folder, "configured_remote.output.json")
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
            out_msg = json.load(out)
    ...
    def test_non_object_configured_remote(self) -> None:
        ...
        inputFile = os.path.join(
            self.test_folder, "configured_non_object_remote.input.json"
        )
        outputFile = os.path.join(
            self.test_folder, "configured_non_object_remote.output.json"
        )
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
    ...
    def test_sfn(self) -> None:
        """test sfn.input.json"""
        inputFile = os.path.join(self.test_folder, "sfn.input.json")
        outputFile = os.path.join(self.test_folder, "sfn.output.json")
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
            out_msg = json.load(out)
    ...
    def test_context(self) -> None:
        ...
        inputFile = os.path.join(self.test_folder, "context.input.json")
        outputFile = os.path.join(self.test_folder, "context.output.json")
        ctxFile = os.path.join(self.test_folder, "lambda-context.json")
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
            out_msg = json.load(out)
        with open(ctxFile, encoding="utf-8") as ctx:
            context = json.load(ctx)
    ...
    def test_inline_template(self) -> None:
        ...
        inputFile = os.path.join(self.test_folder, "inline_template.input.json")
        outputFile = os.path.join(self.test_folder, "inline_template.output.json")
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
            out_msg = json.load(out)
    ...
    def test_templates(self) -> None:
        ...
        inputFile = os.path.join(self.test_folder, "templates.input.json")
        outputFile = os.path.join(self.test_folder, "templates.output.json")
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
            out_msg = json.load(out)
    ...
    def test_cumulus_context(self) -> None:
        ...
        inputFile = os.path.join(self.test_folder, "cumulus_context.input.json")
        outputFile = os.path.join(self.test_folder, "cumulus_context.output.json")
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
        with open(outputFile, encoding="utf-8") as out:
            out_msg = json.load(out)
    ...
    def test_input_jsonschema(self) -> None:
        ...
        inputFile = "templates.input.json"
        with open(inputFile, encoding="utf-8") as inp:
            in_msg = json.load(inp)
    ...
    def test_failing_config_jsonschema(self) -> None:
        ...
        try:
            ...
            assert (
                e.message == "config schema: 'notgoingtowork' is not of type u'boolean'"
            )
