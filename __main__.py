...
if __name__ == '__main__':
    ...
    try:
        if (functionName == 'loadRemoteEvent'):
            result = transformer.loadRemoteEvent(event, schemas)
        elif (functionName == 'loadNestedEvent'):
            context = allInput['context']
            result = transformer.loadNestedEvent(event, context, schemas)
            ...
            if 'message_config' in allInput:
                ...
                messageConfig = None
            result = transformer.createNextEvent(handlerResponse, event, messageConfig, schemas)
